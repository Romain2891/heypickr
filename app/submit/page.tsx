...
export default function SubmitPage() {
  ...
  if (success) return (
    <main style={{ minHeight: '100svh', backgroundColor: '#080808', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, fontFamily: 'system-ui', padding: 24 }}>
      ...
      <div style={{ fontFamily: '"Bebas Neue", sans-serif', fontSize: 28, letterSpacing: 4, color: '#fff' }}>Soumis !</div>
      <div style={{ fontSize: 13, color: '#555', letterSpacing: 1, textAlign: 'center' }}>La communauté va voter sur ton pickr.</div>
      <a href="/vote" style={{ background: '#C9A84C', color: '#080808', padding: '10px 28px', borderRadius: 20, fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', fontWeight: 600, textDecoration: 'none' }}>Voir les votes →</a>
    </main>
  )
  ...
  return (
    <main style={{ minHeight: '100svh', backgroundColor: '#080808', color: '#fff', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ padding: '10px 20px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        <a href="/vote" style={{ color: '#444', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', textDecoration: 'none' }}>← retour</a>
      </div>

      <div style={{ padding: '0 20px 16px', borderBottom: '0.5px solid #1a1a1a', flexShrink: 0 }}>
        <div style={{ fontFamily: '"Bebas Neue", "Arial Black", sans-serif', fontSize: 15, letterSpacing: 5, color: '#555' }}>NOUVEAU PICKR</div>
      </div>
      ...
      <div style={{ flex: 1, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div>
          <div style={{ fontSize: 10, letterSpacing: 3, color: '#C9A84C', textTransform: 'uppercase', marginBottom: 12 }}>Catégorie</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setCategory(cat)} style={{ padding: '6px 14px', borderRadius: 20, fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', background: category === cat ? '#C9A84C' : 'transparent', color: category === cat ? '#080808' : '#444', border: category === cat ? '0.5px solid #C9A84C' : '0.5px solid #2a2a2a', fontWeight: category === cat ? 600 : 400 }}>
                {cat}
              </button>
            ))}
          </div>
        ...
        <div style={{ background: '#111', border: '0.5px solid #1a1a1a', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#444', letterSpacing: 1 }}>Coût</span>
      ...
      <div style={{ borderTop: '0.5px solid #1a1a1a', display: 'flex', justifyContent: 'space-around', padding: '7px 0 4px', flexShrink: 0, background: '#080808' }}>
        {[
          ...
          { href: '/submit', label: 'Créer', active: true, icon: 'M12 5v14M5 12h14' },
