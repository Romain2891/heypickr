...
export default function SubmitPage() {
  ...
  if (success) return (
    <main style={{ minHeight: '100svh', backgroundColor: '#080808', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24, fontFamily: 'system-ui', padding: 24 }}>
      ...
      <div style={{ fontFamily: 'Bebas Neue, Arial Black, sans-serif', fontSize: 28, letterSpacing: 4, color: '#fff' }}>Submitted!</div>
      <div style={{ fontSize: 13, color: '#fff', letterSpacing: 1, textAlign: 'center' }}>The community will vote on your pickr.</div>
  ...
  return (
    <main style={{ minHeight: '100svh', backgroundColor: '#080808', color: '#fff', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '10px 20px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        ...
        <button onClick={goToVote} style={{ color: '#fff', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', background: 'none', border: 'none', cursor: 'pointer' }}>← back</button>
      </div>
      ...
      <div style={{ flex: 1, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {(['A', 'B'] as const).map(label => (
            <label key={label} style={{ aspectRatio: '1', background: '#111', border: '0.5px solid #333', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, cursor: 'pointer', overflow: 'hidden', position: 'relative' }}>
              ...
              {(label === 'A' ? previewA : previewB) ? (
                ...
                <>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M17 8l-5-5-5 5M12 3v12" /></svg>
                  <span style={{ fontSize: 10, letterSpacing: 3, color: '#888', textTransform: 'uppercase' }}>photo {label}</span>
                </>
              )}
            </label>
          ))}
        ...
        <div style={{ background: '#111', border: '0.5px solid #1a1a1a', borderRadius: 12, padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: 11, color: '#fff', letterSpacing: 1 }}>Cost</span>
          <span style={{ fontSize: 11, color: '#C9A84C', letterSpacing: 1, fontWeight: 600 }}>1 credit</span>
        </div>

        <button onClick={handleSubmit} disabled={loading || !photoA || !photoB} style={{ background: loading || !photoA || !photoB ? '#1a1a1a' : '#C9A84C', color: loading || !photoA || !photoB ? '#888' : '#080808', border: 'none', borderRadius: 20, padding: '14px', fontSize: 12, letterSpacing: 3, textTransform: 'uppercase', fontWeight: 600, cursor: loading || !photoA || !photoB ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}>
      ...
      <div style={{ borderTop: '0.5px solid #1a1a1a', display: 'flex', justifyContent: 'space-around', padding: '7px 0 4px', flexShrink: 0, background: '#080808' }}>
        {nav.map(item => (
          <button key={item.label} onClick={() => window.location.href = item.path} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, color: item.label === 'Submit' ? '#C9A84C' : '#fff', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase', padding: '3px 12px', background: 'none', border: 'none', cursor: 'pointer' }}>
