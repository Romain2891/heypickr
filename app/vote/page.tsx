...
export default function VotePage() {
  ...
  const nav = [
    { label: 'Vote', active: true, icon: 'M4 6h16M4 12h16M4 18h7', path: '/vote' },
    { label: 'Submit', active: false, icon: 'M12 5v14M5 12h14', path: '/submit' },
    { label: 'Results', active: false, icon: 'M4 20V10m5 10V4m5 16v-7m5 7v-4', path: '/results' },
    { label: 'Profile', active: false, icon: 'M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2M12 11a4 4 0 100-8 4 4 0 000 8z', path: '/profile' },
  ]
  ...
  return (
    <main style={{ minHeight: '100svh', backgroundColor: '#080808', color: '#fff', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, minHeight: 0 }}>
        {pickr ? (
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', gap: 16, padding: 24 }}>
            ...
            <button onClick={() => window.location.href = '/submit'} style={{ background: '#C9A84C', color: '#080808', padding: '10px 24px', borderRadius: 20, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase' as const, fontWeight: 600, border: 'none', cursor: 'pointer' }}>Submit the first one →</button>
          </div>
        )}
      </div>
      ...
      <div style={{ borderTop: '0.5px solid #1a1a1a', display: 'flex', justifyContent: 'space-around', padding: '7px 0 4px', flexShrink: 0, background: '#080808' }}>
        {nav.map(item => (
          <button key={item.label} onClick={() => window.location.href = item.path} style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 3, color: item.active ? '#C9A84C' : '#fff', fontSize: 9, letterSpacing: 2, textTransform: 'uppercase' as const, padding: '3px 12px', background: 'none', border: 'none', cursor: 'pointer' }}>
